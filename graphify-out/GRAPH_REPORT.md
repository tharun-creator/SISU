...
Cohesion: 0.05
Nodes (100): _agents_install(), _agents_uninstall(), _always_on(), _amp_install(), _amp_legacy_cleanup(), _amp_uninstall(), _antigravity_finalize(), _antigravity_install() (+92 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (81): _agents_install(), _agents_uninstall(), _install(), _kilo_install(), _kilo_uninstall(), Tests for graphify install --platform routing., Codex skill file must reference spawn_agent., Codex skill must keep graph-first orientation in the lean-core split.      The (+73 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (84): classify_file(), Return the interpreter name from a shebang line.      Handles forms that a nai, _shebang_interpreter(), `-S` payload may carry NAME=value assignments before the interpreter., `-S` payload may carry env flags (e.g. -i) before the interpreter., `--split-string=` payload may carry assignments before the interpreter., `--split-string=` payload may carry env flags before the interpreter., A `-S` payload that itself starts with `-S` is rejected (allow_split=False (+76 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (75): detect(), _is_sensitive(), Return True if this file likely contains secrets and should be skipped., No .graphifyignore is not an error., Comment lines in .graphifyignore are not treated as patterns., Without a VCS root, parent .graphifyignore does NOT apply (hermetic)., Inside a VCS repo, parent .graphifyignore applies to subdirectory scans., Upward search stops at the git repo root (.git directory). (+67 more)

### Community 6 - "Community 6"
Cohesion: 0.05
Nodes (65): _git_root(), _hooks_dir(), install(), _install_hook(), Walk up to find .git directory., Return the git hooks directory, respecting core.hooksPath if set (e.g. Husky)., # NOTE: do NOT pass --path-format=absolute — added in git 2.31; older git, Install a single git hook, appending if an existing hook is present. (+57 more)

### Community 7 - "Community 7"
Cohesion: 0.06
Nodes (68): Namespace, _always_on_constants(), always_on_roundtrip(), _audit_allowlist(), audit_coverage(), bless(), check(), _enum_lines() (+60 more)
...
Cohesion: 0.06
Nodes (66): extract_go(), extract_rust(), Extract functions, methods, type declarations, and imports from a .go file., Extract functions, structs, enums, traits, impl methods, and use declarations fr, Methods on the same receiver type must share one canonical type node., Type node id should be scoped to directory, not file stem., test_go_receiver_methods_share_type_node(), test_go_receiver_uses_pkg_scope() (+58 more)
...
Cohesion: 0.07
Nodes (54): deduplicate_entities(), _entropy(), _is_variant_pair(), _llm_tiebreak(), _make_minhash(), _norm(), _pick_winner(), Entity deduplication pipeline for graphify knowledge graphs.  Pipeline: exact (+46 more)
...
Cohesion: 0.04
Nodes (70): collect_files(), extract_bash(), extract_json(), Extract functions, source imports, and cross-function calls from a .sh file., Extract top-level keys, nested structure, and dependency edges from a .json file, Dart child node IDs must be built from _file_stem rather than absolute path., Same input always produces same output., After merging multiple files, no internal edges should be dangling. (+62 more)

### Community 18 - "Community 18"
Cohesion: 0.10
Nodes (52): _canonical_edge(), _count_extra(), diagnose_extraction(), diagnose_file(), _edge_list(), _exact_signature(), format_diagnostic_json(), format_diagnostic_report() (+44 more)
...
Cohesion: 0.06
Nodes (49): _devin_rules_install(), Write .windsurf/rules/graphify.md for always-on Devin context., _devin_install_user(), Tests for graphify devin install / uninstall commands., The rules file installed by devin must use query-first policy., Installing rules twice does not change content and prints 'no change'., Project-scope install prints a git add hint covering .devin/ and .windsurf/., User-scope uninstall removes the skill file. (+41 more)

### Community 25 - "Community 25"
Cohesion: 0.06
Nodes (46): _build_scip_metadata(), _coerce_str(), _emit_relationships(), _emit_symbol_node(), _first_occurrence_line(), _is_true(), _make_scip_node_id(), scip_ingest.py — SCIP JSON ingestion (simplified subset).  Reads a simplified (+38 more)

### Community 26 - "Community 26"
Cohesion: 0.09
Nodes (45): _community_article(), _cross_community_links(), _god_node_article(), _index_md(), Make a label safe for use as a filename across platforms.      Substitutes cha, Generate a Wikipedia-style wiki from the graph.      Writes:       - index.md, Return (community_label, edge_count) pairs for cross-community connections, sort, _safe_filename() (+37 more)

### Community 27 - "Community 27"
Cohesion: 0.07
Nodes (43): extract_python(), Extract classes, functions, and imports from a .py file via tree-sitter AST., Call-graph pass must produce INFERRED calls edges., AST-resolved call edges are deterministic and should be EXTRACTED/1.0., run_analysis() calls compute_score() - must appear as a calls edge., Analyzer.process() calls run_analysis() - cross class→function calls edge., Same caller→callee pair must appear only once even if called multiple times., All edge sources must reference a known node (targets may be external imports). (+35 more)

### Community 28 - "Community 28"
Cohesion: 0.11
Nodes (43): _backend_env_keys(), _call_bedrock(), _call_claude(), _call_claude_cli(), _call_llm(), _call_openai_compat(), _custom_providers_path(), _default_model_for_backend() (+35 more)

### Community 29 - "Community 29"
Cohesion: 0.05
Nodes (44): ingest_scip_json(), Convert a SCIP-style JSON document into Graphify nodes and edges.      Paramet, Cross-document relationship resolves to the target document's node id., A relationship targeting a symbol NOT in any document creates a stub external no, `relative_path` as a non-string falls back to the function's source_file default, A symbol with `relationships: None` ingests without error and emits no edges., A relationship entry whose `symbol` is a non-string is silently skipped., A non-dict entry in `documents` is silently skipped. (+36 more)
...
Cohesion: 0.05
Nodes (41): Comprehensive tests for graphify.scip_ingest., Symbol ending with '#' and no display_name must produce a non-empty label., A symbol with `kind` as a non-string falls back to 'unknown'., `documentation` first entry that isn't a string yields empty description (not cr, Top-level non-dict shapes still return the empty result., When two docs both have `F#`, a relationship from b.py's F# to F# must     reso, When a target symbol is defined in 2+ documents AND the source is in a     thir, `"is_implementation": "false"` is a truthy STRING — must not route to     scip_ (+33 more)

### Community 33 - "Community 33"
Cohesion: 0.05
Nodes (41): extract_js(), Extract classes, functions, arrow functions, and imports from a .js/.ts/.tsx fil, export { localVar } without 'from' should NOT create re_exports edges., `const { foo } = require('./mod')` must emit imports_from to the resolved module, Destructured CJS requires must emit symbol-level `imports` edges per binder., `const x = require('./m').y` must emit symbol edge for `y`., Regression: arrow functions in lexical_declaration must still produce nodes., Functions defined alongside a JSX-returning component must be captured. (+33 more)
...
Cohesion: 0.08
Nodes (37): build_whisper_prompt(), download_audio(), _get_whisper(), _get_yt_dlp(), is_url(), _model_name(), Transcribe a video/audio file or URL to a .txt transcript.      If video_path, Transcribe a list of video/audio files or URLs, return paths to transcript .txt (+29 more)

### Community 38 - "Community 38"
Cohesion: 0.11
Nodes (39): _make_graph(), Integration tests for graphify export subcommands and CLI commands.  Each test, cluster-only must not crash with FileNotFoundError when graphify-out/ is absent, cluster-only must invoke remap_communities_to_previous so the existing     .gra, Build a minimal graph.json + analysis/labels files in tmp_path/graphify-out/., When .graphify_analysis.json is absent, export html should reconstruct     comm, Stronger assertion: the reconstructed `communities` dict should have the     SA, If a graph.json was somehow written without any per-node `community`     attrib (+31 more)

### Community 39 - "Community 39"
Cohesion: 0.13
Nodes (37): _estimate_tokens(), _hr(), print_benchmark(), _query_subgraph_tokens(), Token-reduction benchmark - measures how much context graphify saves vs naive fu, Print a human-readable benchmark report., Return unicode_char if stdout can encode it, else ascii_fallback.      Windows, Horizontal rule that survives non-UTF-8 stdout (e.g. Windows cp1252 console). (+29 more)
...
Cohesion: 0.07
Nodes (37): _estimate_file_tokens(), extract_corpus_parallel(), _merge_into(), _pack_chunks_by_tokens(), Extract a corpus in chunks, merging results.      Chunking strategy:, Append a chunk result into the running merged accumulator., Estimate the prompt-token cost of a single file under `_read_files` rules., Greedily pack files into chunks that fit a token budget.      Files are first (+29 more)

### Community 43 - "Community 43"
Cohesion: 0.14
Nodes (13): AsyncClient, BaseClient, Client, The main Client and AsyncClient classes. BaseClient holds all shared logic. Cli, Asynchronous HTTP client., Shared implementation for Client and AsyncClient.     Handles auth, redirects,, Synchronous HTTP client., InvalidURL (+5 more)
...
Cohesion: 0.08
Nodes (25): _clear_backend_env(), _fake_openai_response(), _install_capturing_openai(), _install_fake_openai(), _ok(), Tests for direct semantic-extraction backend selection., Build a minimal stand-in for an `openai` SDK ChatCompletion response., Inject a stub `openai` module so `_call_openai_compat` can run without     the (+17 more)

### Community 46 - "Community 46"
Cohesion: 0.07
Nodes (36): _build_wheel_names(), fake_bundle(), _first_unbuilt_progressive_host(), _install(), Tests for the progressive-disclosure references/ sidecar install path.  The re, Reinstall swaps references/ in place, dropping a stale fragment., Uninstall rmtrees references/ before the dir walk so the tree is cleared., If SKILL.md links references/ but the dir is gone, warn to repair. (+28 more)
...
Cohesion: 0.11
Nodes (28): _log_path(), log_query(), _log_responses(), nodes_from_result(), Query logging for graphify — append-only JSONL, fail-silent., Append one JSONL record to the query log. Never raises., Tests for graphify.querylog., test_disable_env() (+20 more)
...
Cohesion: 0.21
Nodes (18): convert_google_workspace_file(), _extract_file_id_from_url(), _extract_resource_key(), google_workspace_enabled(), Optional Google Workspace shortcut export support.  Google Drive for desktop s, Export a Google Workspace shortcut to a Markdown sidecar.      Returns the con, Return True when Google Workspace shortcut export is enabled., Extract a Drive file ID from common Google Docs/Drive URL shapes. (+10 more)

### Community 73 - "Community 73"
Cohesion: 0.15
Nodes (14): _classify(), format_prs_text(), PRInfo, Plain-text PR summary for MCP output (no ANSI)., PRInfo, make_pr(), Tests for graphify/prs.py., Build a minimal PRInfo with sensible defaults. (+6 more)
...
Cohesion: 0.07
Nodes (24): _notify_only(), Write a flag file and print a notification (fallback for non-code-only corpora)., Tests for watch.py - file watcher helpers (no watchdog required)., When the caller supplies an absolute path, ``.graphify_root`` stores     that a, #1007: graphify update (_rebuild_code with no changed_paths) must remove     no, gh-928: the watch Handler must short-circuit paths matching     .graphifyignore, gh-928: .graphifyignore must be parsed exactly once at watch() startup,     not, End-to-end probe of the post-commit-delete bug fix.      Build a tiny graph, d (+16 more)
...
Cohesion: 0.10
Nodes (24): _load_graph(), Tests for serve.py - MCP graph query helpers (no mcp package required)., Write a minimal graph.json with the given node IDs., serve() picks up a new graph.json written after startup (#874)., mtime_ns + size uniquely identifies a graph version (#874)., Chinese text should use the cached jieba module and keep the original term., Mixed Chinese and English text should be handled correctly., Japanese kana and Hangul are kept as terms but not segmented as Chinese. (+16 more)
...
Cohesion: 0.34
Nodes (23): bold(), _c(), _ci_icon(), cmd_prs(), cyan(), dim(), green(), magenta() (+15 more)
...
Cohesion: 0.17
Nodes (22): _community_label_lines(), generate_community_labels(), label_communities(), _parse_label_response(), _placeholder_community_labels(), One prompt line per community (largest first), sampling up to ``top_k``     rep, Parse the backend's JSON ``{cid: name}`` reply. Raises on non-JSON or a     non, Return a complete ``{cid: name}`` map using ``backend`` for naming.      Place (+14 more)
...
Cohesion: 0.11
Nodes (22): build_label_index(), build_python_symbol_index(), existing_edge_pairs(), node_is_resolvable_symbol(), _node_source_stem(), normalise_callable_label(), Return the stem of a node's source file., Build ``(module_stem, normalized_symbol_name) -> node_ids``.      This index i (+14 more)

### Community 123 - "Community 123"
Cohesion: 0.10
Nodes (19): _make_envelope(), Tests for `_parse_llm_json` robustness and the `_call_claude_cli` subprocess ar, The hollow-response root cause was --append-system-prompt     layering graphify, GRAPHIFY_CLAUDE_CLI_MODEL must be forwarded to claude -p --model., Default behaviour: when the env var is not set, --model is not     added so cla, Claude often prefixes the JSON with a short preamble before the     ```json fen, Some models return prose around bare JSON with no markdown fence.     The balan, Regression: clean JSON input (the original happy path) must keep     parsing ex (+11 more)
...
Cohesion: 0.20
Nodes (20): cluster(), cohesion_score(), _partition(), Community detection on NetworkX graphs. Uses Leiden (graspologic) if available,, Context manager to suppress stdout/stderr during library calls.      graspolog, Run a second Leiden pass on a community subgraph to split it further., Ratio of actual intra-community edges to maximum possible., Remap community IDs to maximize overlap with a previous assignment.      Uses (+12 more)

### Community 129 - "Community 129"
Cohesion: 0.20
Nodes (20): _filter_graph_by_context(), _find_node(), _has_chinese(), _infer_context_filters(), _is_searchable(), _normalize_context_filters(), _query_terms(), Return node IDs whose label or ID matches the search term (diacritic-insensitive (+12 more)
...
Cohesion: 0.12
Nodes (20): derive_flow_chain(), generate_overview_cards(), is_zh(), node_degree_scores(), node_in_section(), preferred_edges(), Aggregate inter-section edge counts and relation names., Derive a readable section flow from inter-section edges. (+12 more)

### Community 132 - "Community 132"
Cohesion: 0.13
Nodes (20): _describe_node(), format_node_refs(), generate_call_table_rows(), generate_section_cards(), group_nodes_by_file(), node_kind(), pick_text(), Group selected nodes by source file for Mermaid subgraphs. (+12 more)

### Community 133 - "Community 133"
Cohesion: 0.14
Nodes (20): _is_ignored(), _load_graphifyignore(), Read .graphifyignore files and return (anchor_dir, pattern) pairs.      Patter, Return True if the path should be ignored per .graphifyignore patterns.      U, A ! re-include cannot un-ignore a file whose parent dir is excluded (#882)., A ! re-include must still un-ignore a file when no ancestor is excluded (#882)., If the ancestor dir itself is re-included, its children should not be blocked (#, /inbox/ must not match src/inbox/ — only inbox/ at the anchor root. (+12 more)

### Community 134 - "Community 134"
Cohesion: 0.16
Nodes (20): _bfs(), _dfs(), Render subgraph as text, cutting at token_budget (approx 3 chars/token)., _subgraph_to_text(), _make_graph(), Truncation message must tell Claude what to do, not just say truncated., test_bfs_depth_1(), test_bfs_depth_2() (+12 more)
...
Cohesion: 0.15
Nodes (18): CallflowOptions, infer_project_name(), load_labels(), load_report(), load_sections(), main(), Options for call-flow architecture HTML generation., Generate call-flow architecture HTML from graphify output files. (+10 more)
...
Cohesion: 0.12
Nodes (19): _query_graph_text(), _make_noisy_graph(), 20 error-handler nodes + 1 rare identifier: FooBarService., error' matches 20 nodes, 'foobarservice' matches 1 — IDF should make     FooBar, A term matching most nodes should get IDF < 1., FooBarService error handling' should expand from FooBarService,     not from er, Searching for '路由' should match a node with label containing '路由'., Full pipeline: '页面路由' should find nodes with '路由' in label. (+11 more)

### Community 140 - "Community 140"
Cohesion: 0.18
Nodes (18): _bash_file_stem(), _bash_make_id(), _file_node_id_for_path(), find_unique_python_symbol(), ImportedSymbol, _module_stem(), parse_python_import_aliases(), Deterministic symbol indexing and conservative cross-file resolution helpers. (+10 more)

### Community 141 - "Community 141"
Cohesion: 0.14
Nodes (19): _check_shrink(), Return True (ok to proceed) or False (shrink refused).      When False, cleans, Build a minimal graph-data dict with *n* placeholder nodes., Default case: smaller new graph + no force + no declared deletions = refuse., force=True bypasses the guard regardless of node delta., Caller declared deletions → shrink is expected → guard skipped silently., First-run case: no existing graph → guard inert., new > existing is always fine. (+11 more)

### Community 142 - "Community 142"
Cohesion: 0.20
Nodes (18): delete_record(), _ensure_storage(), list_records(), load_index(), load_record(), Storage module - persists documents to disk and maintains the search index. All, Load the full document index from disk., Persist the index to disk. (+10 more)

### Community 143 - "Community 143"
Cohesion: 0.17
Nodes (18): _assert_no_report_first(), _assert_query_first(), Installer-level regression tests for upgrade-in-place behavior (issue #580)., The Claude install must also rewrite a stale .claude/settings.json hook     pay, Same upgrade behavior for AGENTS.md (Codex / OpenCode / Aider / Trae)., Same upgrade behavior for GEMINI.md., Same upgrade behavior for .github/copilot-instructions.md (VS Code)., Same upgrade behavior for .cursor/rules/graphify.mdc.     The Cursor rule file (+10 more)

### Community 144 - "Community 144"
Cohesion: 0.18
Nodes (18): DiGraph, find_import_cycles(), Detect circular import dependencies at the file level.      Collapses symbol-l, _make_cycle_graph_directed(), _make_file_node(), npm package.json dep-block keys must be filtered from god_nodes output.      C, Create a graph node resembling real graphify schema., test_find_import_cycles_detects_2_and_3_cycles() (+10 more)

### Community 145 - "Community 145"
Cohesion: 0.11
Nodes (17): FILESYSTEM_ROOT, GITHUB_PERSONAL_ACCESS_TOKEN, args, command, args, command, env, args (+9 more)
...
Cohesion: 0.16
Nodes (8): extract_dart(), Extract classes, mixins, functions, imports, generic calls, and annotations from, Test that the universal parser successfully extracts generic relationships, anno, Test complex Dart 3+ syntax and precise Riverpod/Bloc mappings., Test that the parser successfully handles namespaces in extends/implements, and, Test typedefs, mixin on, factories, constructor DI types, and universal navigati, Test all 5 roadmap bug fixes (Bug A, B, C, D, E)., TestDart

### Community 148 - "Community 148"
Cohesion: 0.23
Nodes (16): generate(), Mirrors export.safe_name so community hub filenames and report wikilinks always, _safe_community_name(), make_inputs(), test_report_contains_ambiguous_section(), test_report_contains_communities(), test_report_contains_corpus_check(), test_report_contains_god_nodes() (+8 more)

### Community 149 - "Community 149"
Cohesion: 0.14
Nodes (20): assert_valid(), Validate an extraction JSON dict against the graphify schema.     Returns a lis, Raise ValueError with all errors if extraction is invalid., validate_extraction(), build(), build_from_json(), Merge multiple extraction results into one graph., test_assert_valid_passes_silently() (+12 more)

### Community 150 - "Community 150"
Cohesion: 0.16
Nodes (19): _apply_resource_limits(), _canonical_graph_for_compare(), _canonical_topology_for_compare(), _git_head(), _json_text(), _node_community_map(), Best-effort nice + memory cap. Called from inline hook scripts.      GRAPHIFY_, Return current git HEAD commit hash, or None outside a repo. (+11 more)
...
Cohesion: 0.12
Nodes (9): SampleUnit, TBaseProcessor, TDataProcessor, Process(), Reset(), IProcessor, TBaseProcessor, IProcessor (+1 more)

### Community 155 - "Community 155"
Cohesion: 0.11
Nodes (22): _build_opener(), _NoFileRedirectHandler, Redirect handler that re-validates every redirect target.      Prevents open-r, Fetch *url* and return raw bytes.      Protections applied:     - URL scheme, Fetch *url* and return decoded text (UTF-8, replacing bad bytes).      Wraps s, Patch socket.getaddrinfo for the duration of a fetch to catch DNS rebinding., safe_fetch(), safe_fetch_text() (+14 more)
...
Cohesion: 0.20
Nodes (15): _append_rationale_attr(), _is_sentence_like_rationale_label(), load_validated_semantic_fragment(), Load and validate a semantic chunk, rejecting oversize files before parsing., Clean up a semantic extraction fragment in-place.      Operations:     1. Rem, Return True if *label* looks like prose / rationale text rather than an     ent, Append one or more rationale strings to *node*'s ``rationale`` attribute., Return validation errors for an untrusted semantic extraction fragment.      E (+7 more)

### Community 165 - "Community 165"
Cohesion: 0.16
Nodes (15): Resolve unqualified raw calls conservatively after all files are known.      T, resolve_cross_file_raw_calls(), Tests for graphify.symbol_resolution., _bash_make_id must produce identical output to _make_id for Unicode inputs., A `from helper import transform` inside a function MUST NOT become     file-wid, A module-level `from helper import transform` IS file-wide evidence., The python cross-file resolver returns [] (not crash) on bad raw_calls., test_bash_make_id_unicode_matches_make_id() (+7 more)

### Community 166 - "Community 166"
Cohesion: 0.18
Nodes (15): enrich_document(), extract_keywords(), find_cross_references(), normalize_text(), process_and_save(), Processor module - transforms validated documents into enriched records ready f, Lowercase, strip extra whitespace, remove control characters., Pull non-stopword tokens from text, deduplicated. (+7 more)

### Community 167 - "Community 167"
Cohesion: 0.12
Nodes (13): Tests for graphify/cache.py., A .md file with no frontmatter is hashed by its full content., Non-.md files are still hashed by their full content., Same file gives same hash on repeated calls., Different file contents give different hashes., Changing only frontmatter fields in a .md file does not change the hash., Changing the body of a .md file produces a different hash., test_file_hash_changes() (+5 more)
...
Cohesion: 0.13
Nodes (15): BaseException, _extract_with_adaptive_retry(), _looks_like_context_exceeded(), Heuristically classify an exception as a context-window overflow.      Differe, Extract a chunk; if the response is truncated (`finish_reason="length"`)     or, No retry when finish_reason='stop' — single call, result passes through., finish_reason='length' triggers split-in-half. Both halves succeed     on the s, When even the half-chunk truncates, split again. With 8 files and a     truncat (+7 more)

### Community 170 - "Community 170"
Cohesion: 0.22
Nodes (10): build_graph(), Graph, Logger, HashMap, Self, T, Processor, Result (+2 more)

### Community 171 - "Community 171"
Cohesion: 0.17
Nodes (15): build_community_index(), _community_text(), derive_sections_from_communities(), generate_section_intro(), _keyword_score(), label_for_community(), _rank_grouped_sections(), Pick representative words from labels and file names. (+7 more)

### Community 172 - "Community 172"
Cohesion: 0.16
Nodes (14): handle_delete(), handle_enrich(), handle_get(), handle_list(), handle_search(), handle_upload(), API module - exposes the document pipeline over HTTP. Thin layer over parser, v, Accept a list of file paths, run the full pipeline on each,     and return a su (+6 more)

### Community 173 - "Community 173"
Cohesion: 0.22
Nodes (14): batch_parse(), parse_and_save(), parse_file(), parse_json(), parse_markdown(), parse_plaintext(), Parser module - reads raw input documents and converts them into a structured f, Read a file from disk and return a structured document. (+6 more)

### Community 174 - "Community 174"
Cohesion: 0.20
Nodes (12): The Read|Glob PreToolUse hook nudges toward the graph instead of raw reads.  C, Reading the graph's own report must not start a go-read-the-graph loop., A nudge is additionalContext only - the hook must exit 0, never deny., _run(), test_glob_pattern_nudges(), test_never_blocks(), test_nudge_payload_is_valid_pretooluse_json(), test_nudges_on_source_read_with_graph() (+4 more)
...
Cohesion: 0.20
Nodes (9): attach_graph_impact(), build_community_labels(), fetch_pr_files(), _load_graph_json(), Return {community_id: [top_labels]} extracted from graph node data., Fetch PR file lists concurrently, compute graph impact, return community labels., TestBuildCommunityLabels, int (+1 more)

### Community 179 - "Community 179"
Cohesion: 0.15
Nodes (14): _compute_idf(), IDF weights for query terms, cached in G.graph['_idf_cache'].      Common term, _score_nodes(), IDF results are stored in G.graph so repeated queries don't recompute., Two separate graph instances must not share an IDF cache., A term matching only 1 of N nodes should get IDF > 1., test_idf_cached_on_graph(), test_idf_new_graph_starts_fresh() (+6 more)
...
Cohesion: 0.38
Nodes (11): affected_nodes(), AffectedHit, format_affected(), _format_location(), load_graph(), _node_label(), resolve_seed(), Graph (+3 more)
...
Cohesion: 0.33
Nodes (5): compute_pr_impact(), Return (communities_touched, nodes_affected) for a set of changed files., 3 nodes across 2 communities, 2 distinct source files., TestComputePrImpact, Graph

### Community 203 - "Community 203"
Cohesion: 0.39
Nodes (11): build_tree(), _common_root(), emit_html(), _make_truncation_leaf(), tree_html — emit a D3 v7 collapsible-tree HTML view of a graph.  A self-contai, Build a ``{name, total_count, children}`` hierarchy.      Each leaf is either, write_tree_html(), Any (+3 more)
...
Cohesion: 0.27
Nodes (8): area(), Circle, describe(), Float64, LinearAlgebra, Geometry, Point, Shape
...
Cohesion: 0.18
Nodes (11): _pick_seeds(), Select BFS seed nodes, stopping when score drops too far below the top.      P, FooBarService at 1000 vs error nodes at 1.0 → only 1 seed chosen., When all scores are within 20% of the top, keep up to 3 seeds., Never return more than max_k seeds even when all scores are close., test_pick_seeds_close_scores_keeps_multiple(), test_pick_seeds_dominant_identifier_gives_one_seed(), test_pick_seeds_empty() (+3 more)

### Community 215 - "Community 215"
Cohesion: 0.18
Nodes (11): Resolve raw Python calls using explicit import evidence.      Only ``from modu, resolve_python_import_guided_calls(), Python import-guided resolver also tolerates malformed raw_calls., A non-dict per_file slot (e.g. a string) must not raise AttributeError., per_file shorter than paths must not raise IndexError., A None per_file slot is treated as empty fragment (no crash, no edges)., test_resolve_python_import_guided_calls_emits_extracted_edge(), test_resolve_python_import_guided_calls_non_dict_per_file_slot() (+3 more)
...
Cohesion: 0.27
Nodes (9): _expected_artifacts(), _has_build(), Packaging guard (#1121 follow-up): the 5 skillgen guards check the *repo tree*,, Every committed references/*.md (per host) + always_on/*.md block., test_skill_artifact_ships_in_wheel(), wheel_namelist(), bool, Path (+1 more)
...
Cohesion: 0.22
Nodes (9): iter_raw_calls(), Return raw calls from all per-file extraction fragments.      Parameter is ``S, A non-dict per_file entry (e.g. junk fragment) must be silently skipped., `raw_calls` that isn't a list must yield empty., Items inside `raw_calls` list that aren't dicts must be dropped., test_iter_raw_calls_drops_non_dict_items_in_list(), test_iter_raw_calls_skips_non_dict_per_file_entries(), test_iter_raw_calls_skips_non_list_raw_calls() (+1 more)
...
Cohesion: 0.36
Nodes (4): _path_match(), True if graph_src and pr_file refer to the same file (path-boundary safe)., TestPathMatch, bool
...
Cohesion: 0.32
Nodes (7): _make_corpus(), Tests for `graphify extract` CLI dispatch path in graphify.__main__., Minimal corpus: one Go code file + one Markdown doc.      Both file types are, When every semantic chunk errors (e.g. backend SDK not installed),     the CLI, Sanity counter-test: a successful chunk run keeps exit 0. Confirms the     new, test_extract_exits_nonzero_when_all_semantic_chunks_fail(), test_extract_succeeds_when_at_least_one_chunk_completes()
...
Cohesion: 0.47
Nodes (5): make_rect(), process(), validate(), Rectangle, main()
...
Cohesion: 0.33
Nodes (6): _cursor_uninstall(), Remove .cursor/rules/graphify.mdc., cursor uninstall removes the rule file., cursor uninstall does nothing if rule was never written., test_cursor_uninstall_noop_if_not_installed(), test_cursor_uninstall_removes_rule()
...
Cohesion: 0.33
Nodes (5): Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal, For /graphify explain, For /graphify path, graphify reference: query, path, explain

### Community 338 - "Community 338"
Cohesion: 0.33
Nodes (6): Build a deterministic fake extraction result for a chunk., Build a stub extraction result with a controllable finish_reason., _stub_chunk_result(), _stub_with_finish(), int, str
...
Cohesion: 0.40
Nodes (5): FCL, SamplePackage, LCL, sampleutils, sample
...
Cohesion: 0.50
Nodes (4): _devin_rules_uninstall(), Remove .windsurf/rules/graphify.md., _devin_rules_uninstall does nothing if the rules file was never written., test_devin_rules_uninstall_noop_when_not_installed()

### Community 413 - "Community 413"
Cohesion: 0.50
Nodes (4): _merge_changed_paths(), Concatenate path lists, preserving order and dropping duplicates.      Used to, _merge_changed_paths preserves first-seen order and drops dupes., test_merge_changed_paths_dedupes_in_order()
...
Cohesion: 0.50
Nodes (3): For native AGENTS.md integration, For git commit hook, graphify reference: commit hook and native AGENTS.md integration

### Community 417 - "Community 417"
Cohesion: 0.50
Nodes (3): For native AGENTS.md integration (Trae), For git commit hook, graphify reference: commit hook and native AGENTS.md integration
...
Cohesion: 0.83
Nodes (3): test_affected_cli_relation_filter_limits_reverse_traversal(), test_affected_cli_reverse_traverses_impact_edges(), _write_graph()
