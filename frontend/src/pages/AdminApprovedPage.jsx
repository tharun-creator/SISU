...
export default function AdminApprovedPage() {
  ...
  const [statusFilter, setStatusFilter] = useState('all'); // 'all' | 'approved' | 'completed'
  ...
  // Filter only approved & completed
  const approvedList = useMemo(() => {
    return meetings.filter(m => m.status === 'approved' || m.status === 'completed');
  }, [meetings]);
  ...
  }, [approvedList, searchQuery, statusFilter]);

  // Stats calculations
  const totalApproved = approvedList.filter(m => m.status === 'approved').length;
  const totalCompleted = approvedList.filter(m => m.status === 'completed').length;
  const thisMonthCount = approvedList.filter(m => m.start_time && isThisMonth(parseISO(m.start_time))).length;

  return (
    <Layout title="Approved Bookings">
      <AnimatePresence>
        {selectedMeeting && (
      ...
      <div style={{ position: 'relative' }}>
        <div style={{ position: 'absolute', top: '-10%', left: '20%', width: 600, height: 600, background: 'radial-gradient(circle, rgba(16, 185, 129, 0.04) 0%, transparent 65%)', borderRadius: '50%', filter: 'blur(80px)', pointerEvents: 'none', zIndex: 0 }} />

        {/* Page Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, position: 'relative', zIndex: 1 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--color-text-primary)', margin: 0, fontFamily: 'var(--font-heading)' }}>Approved Bookings</h1>
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginTop: 4, marginBottom: 0 }}>Review, update priorities, or reschedule approved/completed mentorship slots.</p>
          </div>
        </div>

        {/* Stats Row */}
        <div className="layout-grid grid-cols-3" style={{ marginBottom: 28, position: 'relative', zIndex: 1 }}>
          <StatCard icon="check_circle" label="Approved Slots" value={totalApproved} color="var(--color-green)" delay={0} />
          <StatCard icon="task_alt" label="Completed Sessions" value={totalCompleted} color="var(--color-accent)" delay={0.05} />
          <StatCard icon="calendar_month" label="Booked This Month" value={thisMonthCount} color="var(--color-accent-cyan)" delay={0.1} />
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 20, position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: 300 }}>
            ...
            <input 
              ...
              placeholder="Search approved slots..." 
              value={searchQuery}
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} style={{ textAlign: 'center', padding: '64px 0', color: 'var(--color-text-muted)', fontSize: 14 }}>
                      <span className="material-symbols-outlined" style={{ fontSize: 32, marginBottom: 8, display: 'block', color: 'var(--color-text-muted)' }}>check_circle</span>
                      No approved or completed bookings found.
                    </td>
                  </tr>
