      ...
      <div style={{ position: 'relative', minHeight: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1, paddingBottom: 40 }}>
          {loading ? (
            ...
            <div className="admin-decision-split-layout" style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 24, width: '100%', position: 'relative', zIndex: 1, alignItems: 'start' }}>
              {/* Left Column: Queue/List of requests */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxHeight: 'calc(100vh - 220px)', overflowY: 'auto', paddingRight: 6 }}>
                {decisionRequests.map((meeting, idx) => {
                  const isActive = idx === activeIndex;
                  const priorityColor = PRIORITY_COLORS[meeting.priority] || 'var(--color-accent)';
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '16px',
                        borderRadius: 16,
                        border: isActive ? '1.5px solid var(--color-accent)' : '1px solid rgba(255, 255, 255, 0.08)',
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        ...
                        <span style={{ 
                          ...
                          height: 8, 
                          borderRadius: '50%', 
                          background: meeting.status === 'reschedule_requested' ? 'var(--color-accent-orange)' : 'var(--color-amber)' 
                        }} />
                      </div>
                      <p style={{ fontSize: 12.5, color: 'var(--color-text-secondary)', fontWeight: 600, margin: '0 0 8px 0', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                        {meeting.title}
                      </p>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11, color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>
                        <span>{meeting.display_date}</span>
                        <span style={{ textTransform: 'uppercase', color: priorityColor, fontWeight: 700 }}>{meeting.priority}</span>
                      </div>
                    </div>
                  );
                })}
                ...
                {activeMeeting && (
                  <div
                    ...
                    style={{
                      ...
                      width: '100%',
                      borderRadius: 24,
                      background: activeMeeting.client_is_priority ? 'linear-gradient(to bottom, rgba(234, 179, 8, 0.03), var(--color-surface-2))' : 'var(--color-surface-2)',
                      border: activeMeeting.client_is_priority ? '1px solid rgba(234, 179, 8, 0.25)' : '1px solid rgba(255,255,255,0.08)',
                      boxShadow: activeMeeting.client_is_priority ? 'var(--shadow-xl), 0 10px 40px rgba(234, 179, 8, 0.15)' : 'var(--shadow-xl), 0 10px 40px rgba(0,0,0,0.3)',
                      padding: 28,
                      display: 'flex',
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ 
                            ...
                            fontWeight: 800, 
                            color: 'white',
                            boxShadow: activeMeeting.client_is_priority ? '0 4px 14px rgba(234, 179, 8, 0.25)' : '0 4px 14px rgba(59, 130, 246, 0.25)' 
                          }}>
                        ...
                        <span className={`badge-priority badge-priority-${activeMeeting.priority}`} style={{ transform: 'scale(0.95)' }}>
                        ...
                        {actionType === null ? (
                          ...
                          >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                              <span className="badge-status-pill" style={{ background: `${STATUS_COLORS[activeMeeting.status]}12`, color: STATUS_COLORS[activeMeeting.status], border: `1px solid ${STATUS_COLORS[activeMeeting.status]}18` }}>
                                <span style={{ width: 5, height: 5, borderRadius: '50%', background: STATUS_COLORS[activeMeeting.status] }} />
                            ...
                            {/* Title / Description */}
                            <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--color-text-primary)', fontFamily: 'var(--font-heading)', marginBottom: 8, letterSpacing: '-0.01em', lineHeight: 1.3 }}>{activeMeeting.title}</h2>
                            ...
                            {activeMeeting.phone && activeMeeting.phone !== 'N/A' && (
                              <p style={{ fontSize: 12, color: 'var(--color-text-primary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
                                ...
                                Contact Phone: {activeMeeting.phone}
                              </p>
                            )}
                            ...
                            <div style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid var(--color-border)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--color-text-primary)' }}>
                                ...
                                <div>
                                  <p style={{ fontSize: 13, fontWeight: 700 }}>{activeMeeting.display_date || format(parseISO(activeMeeting.start_time), 'EEEE, MMM dd, yyyy')}</p>
                                  <p style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2 }}>{activeMeeting.display_time || format(parseISO(activeMeeting.start_time), 'hh:mm a')} IST</p>
                                </div>
                              </div>
                            </div>
